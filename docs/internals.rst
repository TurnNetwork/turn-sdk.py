...
* Transparently intercept transactions sent over ``bub_sendTransaction``, sign
  them locally, and then send them through ``bub_sendRawTransaction``.
...
.. py:method:: BaseProvider.make_request(method, params)
    ...
    * ``method``  This will be a string representing the JSON-RPC method that
      is being called such as ``'bub_sendTransaction'``.
