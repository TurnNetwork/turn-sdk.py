...
.. code-block:: python
    ...
    >>> Greeter = w3.bub.contract(abi=abi, bytecode=bytecode)
...
Instead, create contract objects using the :meth:`w3.bub.contract() <web3.bub.Bub.contract>`
...
.. py:classmethod:: Contract.constructor(*args, **kwargs).transact(transaction=None)
    ...
    ``web3.bub.send_transaction(transaction)`` method.  This value may not
    ...
    deployment transaction will be created using the ``web3.bub.estimate_gas()``
    method.
    ...
    .. code-block:: python
        ...
        >>> deploy_txn = token_contract.constructor(web3.bub.coinbase, 12345).transact()
        >>> txn_receipt = web3.bub.get_transaction_receipt(deploy_txn)
...
.. py:classmethod:: Contract.constructor(*args, **kwargs).estimate_gas(transaction=None, block_identifier=None)
    ...
    .. code-block:: python
        ...
        >>> token_contract.constructor(web3.bub.coinbase, 12345).estimate_gas()
        12563
...
.. py:classmethod:: Contract.constructor(*args, **kwargs).build_transaction(transaction=None)
    ...
    .. code-block:: python
        ...
        >>> contract_data = token_contract.constructor(web3.bub.coinbase, 12345).build_transaction(transaction)
        >>> web3.bub.send_transaction(contract_data)
...
.. py:classmethod:: Contract.events.your_event_name.create_filter(fromBlock=block, toBlock=block, \
    ...
    - ``topics`` optional, accepts the standard JSON-RPC topics argument.  See the JSON-RPC documentation for `bub_newFilter <https://github.com/ethereum/wiki/wiki/JSON-RPC#bub_newFilter>`_ more information on the ``topics`` parameters.
...
.. doctest:: arrayscontract
    ...
    >>> arrays_contract_instance = w3.bub.contract(abi=abi, bytecode=bytecode)

    >>> tx_hash = arrays_contract_instance.constructor([b'bb']).transact()
    >>> tx_receipt = w3.bub.wait_for_transaction_receipt(tx_hash)
    >>> arrays_contract = w3.bub.contract(
    ...     address=tx_receipt.contractAddress,
    ...     abi=abi
    ... )
...
For example:
    ...
    .. code-block:: python
        ...
        myContract = web3.bub.contract(address=contract_address, abi=contract_abi)
...
"preflighting" with an ``bub_call``, handling the ``OffchainLookup``, and sending the data via a transaction may be
...
.. py:method:: ContractFunction.transact(transaction)
    ...
    the same format as the ``web3.bub.send_transaction(transaction)`` method.
    ...
    If a ``gas`` value is not provided, then the ``gas`` value for the
    method transaction will be created using the ``web3.bub.estimate_gas()``
    method.
    ...
    .. code-block:: python
        ...
        >>> token_contract.functions.transfer(web3.bub.accounts[1], 12345).transact()
...
.. py:method:: ContractFunction.call(transaction, block_identifier='latest')
    ...
    ``bub_call`` API.  This will not create a new public transaction.
    ...
    .. code-block:: python
        ...
        21
        >>> token_contract.functions.myBalance().call({'from': web3.bub.coinbase})
        12345  # the token balance for `web3.bub.coinbase`
        >>> token_contract.functions.myBalance().call({'from': web3.bub.accounts[1]})
        54321  # the token balance for the account `web3.bub.accounts[1]`
...
.. py:method:: ContractFunction.estimate_gas(transaction, block_identifier=None)
    ...
    .. note::
        The parameter ``block_identifier`` is not enabled in bub nodes,
        hence passing a value of ``block_identifier`` when connected to a bub
...
.. py:method:: ContractFunction.build_transaction(transaction)
    ...
    .. note::
        ...
        You may use :meth:`~web3.bub.Bub.getTransactionCount` to get the current nonce
        for an account. Therefore a shortcut for producing a transaction dictionary with
        nonce included looks like:

        .. code-block:: python

            >>> math_contract.functions.increment(5).build_transaction({'nonce': web3.bub.get_transaction_count('0xF5...')})
    ...
    :meth:`~web3.bub.account.Account.sign_transaction`.
...
For example:
    ...
    .. code-block:: python
        ...
        receipt = web3.bub.get_transaction_receipt(tx_hash)
...
.. testsetup:: create_filter
    ...
    my_token_contract = w3.bub.contract(abi=ABI, bytecode=bytecode)
...
.. py:classmethod:: Contract.decode_function_input(data)
    ...
    .. code-block:: python
        ...
        >>> transaction = w3.bub.get_transaction('0x5798fbc45e3b63832abc4984b0f3574a13545f415dd672cd8540cd71f735db56')
        >>> transaction.input
...
.. testsetup:: contractcaller
   ...
   contract = w3.bub.contract(abi=ABI, bytecode=bytecode)
...
.. doctest:: contractcaller
   ...
   >>> myContract = w3.bub.contract(address=address, abi=ABI)
...
.. doctest:: contractcaller
   ...
   >>> from_address = w3.bub.accounts[1]
