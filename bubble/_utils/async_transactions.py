...
async def _estimate_gas(async_w3: "AsyncWeb3", tx: TxParams) -> int:
    return await async_w3.bub.estimate_gas(tx)
...
async def _max_priority_fee_gas(async_w3: "AsyncWeb3", _tx: TxParams) -> Wei:
    return await async_w3.bub.max_priority_fee


async def _chain_id(async_w3: "AsyncWeb3", _tx: TxParams) -> int:
    return await async_w3.bub.chain_id
...
async def get_block_gas_limit(
    web3_eth: "AsyncBub", block_identifier: Optional[BlockIdentifier] = None
) -> int:
...
async def get_buffered_gas_estimate(
    ...
    gas_estimate = await async_w3.bub.estimate_gas(gas_estimate_transaction)
...
async def fill_transaction_defaults(
    ...
    """
    strategy_based_gas_price = async_w3.bub.generate_gas_price(transaction)
...
async def async_get_required_transaction(
    ...
    current_transaction = await async_w3.bub.get_transaction(transaction_hash)
...
async def async_replace_transaction(
    ...
    return await async_w3.bub.send_transaction(new_transaction)
