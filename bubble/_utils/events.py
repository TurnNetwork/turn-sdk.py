...
if TYPE_CHECKING:
    ...
    from bubble._utils.filters import (  # noqa: F401
        AsyncLogFilter,
        LogFilter,
    )
...
class EventFilterBuilder(BaseEventFilterBuilder):
    def deploy(self, w3: "Web3") -> "LogFilter":
        if not isinstance(w3, bubble.Web3):
            raise ValueError(f"Invalid bubble argument: got: {w3!r}")

        for arg in AttributeDict.values(self.args):
            arg._immutable = True
        self._immutable = True

        log_filter = cast("LogFilter", w3.bub.filter(self.filter_params))
...
class AsyncEventFilterBuilder(BaseEventFilterBuilder):
    async def deploy(self, async_w3: "AsyncWeb3") -> "AsyncLogFilter":
        if not isinstance(async_w3, bubble.AsyncWeb3):
            raise ValueError(f"Invalid bubble argument: got: {async_w3!r}")

        for arg in AttributeDict.values(self.args):
            arg._immutable = True
        self._immutable = True

        log_filter = await async_w3.bub.filter(self.filter_params)
