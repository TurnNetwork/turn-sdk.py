...
class GoEthereumAdminModuleTest:
    ...
    def test_admin_peers(self, w3: "Web3") -> None:
        enode = w3.node.admin.node_info()["enode"]
        w3.node.admin.add_peer(enode)
        result = w3.node.admin.peers()
        assert len(result) == 1

    def test_admin_start_stop_http(self, w3: "Web3") -> None:
        stop = w3.node.admin.stop_http()
        assert stop is True

        start = w3.node.admin.start_http()
        assert start is True

    def test_admin_start_stop_ws(self, w3: "Web3") -> None:
        stop = w3.node.admin.stop_ws()
        assert stop is True

        start = w3.node.admin.start_ws()
        assert start is True
...
class GoEthereumAsyncAdminModuleTest:
    ...
    async def test_async_datadir(self, async_w3: "AsyncWeb3") -> None:
        datadir = await async_w3.node.admin.datadir()
        assert isinstance(datadir, str)

    @pytest.mark.asyncio
    async def test_async_node_info(self, async_w3: "AsyncWeb3") -> None:
        node_info = await async_w3.node.admin.node_info()
        assert "Bub" in node_info["name"]

    @pytest.mark.asyncio
    async def test_async_nodes(self, async_w3: "AsyncWeb3") -> None:
        nodes = await async_w3.node.admin.peers()
        assert isinstance(nodes, List)

    @pytest.mark.asyncio
    async def test_admin_peers(self, async_w3: "AsyncWeb3") -> None:
        node_info = await async_w3.node.admin.node_info()
        await async_w3.node.admin.add_peer(node_info["enode"])
        result = await async_w3.node.admin.peers()
        assert len(result) == 1

    @pytest.mark.asyncio
    async def test_admin_start_stop_http(self, async_w3: "AsyncWeb3") -> None:
        stop = await async_w3.node.admin.stop_http()
        assert stop is True

        start = await async_w3.node.admin.start_http()
        assert start is True

    @pytest.mark.asyncio
    async def test_admin_start_stop_ws(self, async_w3: "AsyncWeb3") -> None:
        stop = await async_w3.node.admin.stop_ws()
        assert stop is True

        start = await async_w3.node.admin.start_ws()
        assert start is True
