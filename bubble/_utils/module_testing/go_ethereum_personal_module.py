...
class GoEthereumPersonalModuleTest:
    ...
    def test_personal_send_transaction(
        ...
        assert (
            w3.bub.get_balance(unlockable_account_dual_type) > constants.WEI_PER_ETHER
        )
        txn_params: TxParams = {
        ...
        txn_hash = w3.node.personal.send_transaction(txn_params, unlockable_account_pw)
        assert txn_hash
        transaction = w3.bub.get_transaction(txn_hash)
    ...
    def test_personal_sign_and_ecrecover(
        ...
        message = "test-bubble-bub-personal-sign"
        signature = w3.node.personal.sign(
            message, unlockable_account_dual_type, unlockable_account_pw
        )
        signer = w3.node.personal.ec_recover(message, signature)
        assert is_same_address(signer, unlockable_account_dual_type)

    @pytest.mark.xfail(
        reason="personal_sign_typed_data JSON RPC call has not been released in bub"
    )
    def test_personal_sign_typed_data(
        ...
        """
        signature = HexBytes(
            w3.node.personal.sign_typed_data(
...
class GoEthereumAsyncPersonalModuleTest:
    ...
    async def test_async_sign_and_ec_recover(
        ...
        signature = await async_w3.node.personal.sign(
            message, unlockable_account_dual_type, unlockable_account_pw
        )
        address = await async_w3.node.personal.ec_recover(message, signature)
        assert is_same_address(unlockable_account_dual_type, address)

    @pytest.mark.asyncio
    async def test_async_import_key(self, async_w3: "AsyncWeb3") -> None:
        address = await async_w3.node.personal.import_raw_key(
    ...
    async def test_async_list_accounts(self, async_w3: "AsyncWeb3") -> None:
        accounts = await async_w3.node.personal.list_accounts()
        assert len(accounts) > 0

    @pytest.mark.asyncio
    async def test_async_list_wallets(self, async_w3: "AsyncWeb3") -> None:
        wallets = await async_w3.node.personal.list_wallets()
    ...
    async def test_async_new_account(self, async_w3: "AsyncWeb3") -> None:
        ...
        account = await async_w3.node.personal.new_account(passphrase)
    ...
    async def test_async_unlock_lock_account(
        ...
        unlocked = await async_w3.node.personal.unlock_account(
            unlockable_account_dual_type, unlockable_account_pw
        )
        assert unlocked is True
        locked = await async_w3.node.personal.lock_account(unlockable_account_dual_type)
    ...
    async def test_async_send_transaction(
        ...
        response = await async_w3.node.personal.send_transaction(
            tx_params, unlockable_account_pw
        )
        assert response is not None

    @pytest.mark.xfail(
        reason="personal_signTypedData JSON RPC call has not been released in bub"
    )
    @pytest.mark.asyncio
    async def test_async_sign_typed_data(
        ...
        signature = await async_w3.node.personal.sign_typed_data(
            message, unlockable_account_dual_type, unlockable_account_pw
        )
        address = await async_w3.node.personal.ec_recover(
