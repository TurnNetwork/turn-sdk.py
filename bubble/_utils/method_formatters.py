...
BLOCK_FORMATTERS = {
    ...
    "nonce": apply_formatter_if(is_not_null, to_hexbytes(81)),
...
def raise_contract_logic_error_on_revert(response: RPCResponse) -> RPCResponse:
    ...
    if isinstance(data, dict) and response["error"].get("message"):
        raise ContractLogicError(
            f'execution reverted: {response["error"]["message"]}, data: {data}', data=data
        )
    ...
    if data.startswith("Reverted "):
        ...
        if not data.startswith(prefix):
            if data.startswith("Reverted 0x"):
                ...
                raise ContractLogicError(
                    f"execution reverted: {revert_reason}, data: {data}", data=data
                )
            else:
