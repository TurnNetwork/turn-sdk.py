...
# is not automatically available on a bubble instance. To use the `PM` module,
# please enable the package management API on an individual bubble instance.
#
...
class ERC1319Registry(ABC):
    ...
    accessed directly, but rather through the methods on ``bubble.pm``.
    ...
    with `bubble.pm.PM`. Any implementation specific logic should be
    ...
    def __init__(self, address: Address, w3: Web3) -> None:
        """
        Initializes the class with the on-chain address of the registry, and a bubble
        instance connected to the chain where the registry can be found.

        Must set the following properties...

        * ``self.registry``: A `bubble.contract` instance of the target registry.
        * ``self.address``: The address of the target registry.
        * ``self.w3``: The *bubble* instance connected to the chain where the
...
class SimpleRegistry(ERC1319Registry):
    ...
    def __init__(self, address: ChecksumAddress, w3: Web3) -> None:
        ...
        self.registry = w3.bub.contract(address=address, abi=abi)
    ...
    def _release(self, package_name: str, version: str, manifest_uri: str) -> bytes:
        ...
        self.w3.bub.wait_for_transaction_receipt(tx_hash)
...
class PM(Module):
    ...
    def set_registry(self, address: Union[Address, ChecksumAddress, ENS]) -> None:
        """
        Sets the current registry used in ``bubble.pm`` functions that read/write
    ...
    def deploy_and_set_registry(self) -> ChecksumAddress:
        ...
        To tie your registry to an ENS name, use bubble's ENS module, ie.
    ...
    def release_package(
        ...
        Requires ``bubble.PM`` to have a registry set. Requires
        ``bubble.bub.default_account`` to be the registry owner.
    ...
    def _validate_set_registry(self) -> None:
        try:
            ...
            raise EthPMException(
                "bubble.pm does not have a set registry. "
                "Please set registry with either: "
                "bubble.pm.set_registry(address) or "
                "bubble.pm.deploy_and_set_registry()"
            )
        if not isinstance(self.registry, ERC1319Registry):
            raise EthPMException(
                "bubble.pm requires an instance of a subclass of ERC1319Registry "
                "to be set as the bubble.pm.registry attribute. Instead found: "
    ...
    def _validate_set_ens(self) -> None:
        if not self.w3:
            raise InvalidAddress(
                "Could not look up ENS address because no bubble connection available"
            )
        elif not self.w3.ens:
            raise InvalidAddress(
                "Could not look up ENS address because bubble.ens is not set up"
            )
