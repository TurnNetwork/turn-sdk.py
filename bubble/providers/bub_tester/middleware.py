...
request_formatters = {
    # Bub
    RPCEndpoint("bub_getBlockByNumber"): apply_formatters_to_args(
        apply_formatter_if(is_not_named_block, to_integer_if_hex),
    ),
    RPCEndpoint("bub_getFilterChanges"): apply_formatters_to_args(hex_to_integer),
    RPCEndpoint("bub_getFilterLogs"): apply_formatters_to_args(hex_to_integer),
    RPCEndpoint("bub_getTransactionCount"): apply_formatters_to_args(
        identity,
        apply_formatter_if(is_not_named_block, to_integer_if_hex),
    ),
    RPCEndpoint("bub_getBlockTransactionCountByNumber"): apply_formatters_to_args(
        apply_formatter_if(is_not_named_block, to_integer_if_hex),
    ),
    RPCEndpoint("bub_getUncleCountByBlockNumber"): apply_formatters_to_args(
        apply_formatter_if(is_not_named_block, to_integer_if_hex),
    ),
    RPCEndpoint("bub_getTransactionByBlockHashAndIndex"): apply_formatters_to_args(
        identity,
        to_integer_if_hex,
    ),
    RPCEndpoint("bub_getTransactionByBlockNumberAndIndex"): apply_formatters_to_args(
        apply_formatter_if(is_not_named_block, to_integer_if_hex),
        to_integer_if_hex,
    ),
    RPCEndpoint("bub_getUncleByBlockNumberAndIndex"): apply_formatters_to_args(
        apply_formatter_if(is_not_named_block, to_integer_if_hex),
        to_integer_if_hex,
    ),
    RPCEndpoint("bub_newFilter"): apply_formatters_to_args(
        filter_request_transformer,
    ),
    RPCEndpoint("bub_getLogs"): apply_formatters_to_args(
        filter_request_transformer,
    ),
    RPCEndpoint("bub_sendTransaction"): apply_formatters_to_args(
        transaction_request_transformer,
    ),
    RPCEndpoint("bub_estimateGas"): apply_formatters_to_args(
        transaction_request_transformer,
    ),
    RPCEndpoint("bub_call"): apply_formatters_to_args(
        transaction_request_transformer,
        apply_formatter_if(is_not_named_block, to_integer_if_hex),
    ),
    RPCEndpoint("bub_uninstallFilter"): apply_formatters_to_args(hex_to_integer),
    RPCEndpoint("bub_getCode"): apply_formatters_to_args(
        identity,
        apply_formatter_if(is_not_named_block, to_integer_if_hex),
    ),
    RPCEndpoint("bub_getBalance"): apply_formatters_to_args(
        identity,
        apply_formatter_if(is_not_named_block, to_integer_if_hex),
    ),
    # EVM
...
result_formatters: Optional[Dict[RPCEndpoint, Callable[..., Any]]] = {
    RPCEndpoint("bub_getBlockByHash"): apply_formatter_if(
        is_dict,
        compose(block_result_remapper, block_result_formatter),
    ),
    RPCEndpoint("bub_getBlockByNumber"): apply_formatter_if(
        is_dict,
        compose(block_result_remapper, block_result_formatter),
    ),
    RPCEndpoint("bub_getBlockTransactionCountByHash"): apply_formatter_if(
        is_dict,
        transaction_result_remapper,
    ),
    RPCEndpoint("bub_getBlockTransactionCountByNumber"): apply_formatter_if(
        is_dict,
        transaction_result_remapper,
    ),
    RPCEndpoint("bub_getTransactionByHash"): apply_formatter_if(
        is_dict,
        compose(transaction_result_remapper, transaction_result_formatter),
    ),
    RPCEndpoint("bub_getTransactionReceipt"): apply_formatter_if(
        is_dict,
        compose(receipt_result_remapper, receipt_result_formatter),
    ),
    RPCEndpoint("bub_newFilter"): integer_to_hex,
    RPCEndpoint("bub_newBlockFilter"): integer_to_hex,
    RPCEndpoint("bub_newPendingTransactionFilter"): integer_to_hex,
    RPCEndpoint("bub_getLogs"): apply_formatter_if(
        is_array_of_dicts,
        apply_list_to_array_formatter(log_result_remapper),
    ),
    RPCEndpoint("bub_getFilterChanges"): apply_formatter_if(
        is_array_of_dicts,
        apply_list_to_array_formatter(log_result_remapper),
    ),
    RPCEndpoint("bub_getFilterLogs"): apply_formatter_if(
        is_array_of_dicts,
        apply_list_to_array_formatter(log_result_remapper),
    ),
    # EVM
...
bubble_tester_middleware = construct_formatting_middleware(
...
def guess_from(w3: "Web3", _: TxParams) -> ChecksumAddress:
    ...
    elif w3.bub.accounts and len(w3.bub.accounts) > 0:
        return w3.bub.accounts[0]
