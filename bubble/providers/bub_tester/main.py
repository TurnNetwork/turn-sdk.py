...
from bubble.middleware.attrdict import (
    async_attrdict_middleware,
    attrdict_middleware,
)
from bubble.middleware.buffered_gas_estimate import (
...
class AsyncBubereumTesterProvider(AsyncBaseProvider):
    middlewares = (
        async_attrdict_middleware,
        async_buffered_gas_estimate_middleware,
        async_default_transaction_fields_middleware,
    ...
    def __init__(self) -> None:
        ...
        # do not import bub_tester until runtime, it is not a default dependency
        from eth_tester import (
            EthereumTester,
        )

        from bubble.providers.bub_tester.defaults import (
            API_ENDPOINTS,
        )
...
class BubbleTesterProvider(BaseProvider):
    middlewares = (
        attrdict_middleware,
        default_transaction_fields_middleware,
        bubble_tester_middleware,
    ...
    def __init__(
        ...
        if ethereum_tester is None:
            self.bubble_tester = EthereumTester()
        elif isinstance(ethereum_tester, EthereumTester):
            self.bubble_tester = ethereum_tester
        elif isinstance(ethereum_tester, BaseChainBackend):
            self.bubble_tester = EthereumTester(ethereum_tester)
        else:
            raise TypeError(
                "Expected ethereum_tester to be of type `bub_tester.EthereumTester` or "
                "a subclass of `bub_tester.backends.base.BaseChainBackend`, "
        ...
        if api_endpoints is None:
            # do not import bub_tester derivatives until runtime,
    ...
    def make_request(self, method: RPCEndpoint, params: Any) -> RPCResponse:
        return _make_request(method, params, self.api_endpoints, self.bubble_tester)
