...
def get_ipc_socket(ipc_path: str, timeout: float = 2.0) -> socket.socket:
    if sys.platform == "win32":
        ...
        from bubble._utils.windows import (
            NamedPipe,
        )
...
def get_default_ipc_path() -> Optional[str]:
    if sys.platform == "darwin":
        ipc_path = os.path.expanduser(
            os.path.join("~", "Library", "Ethereum", "bub.ipc")
        )
        ...
        ipc_path = os.path.expanduser(os.path.join("~", ".ethereum", "bub.ipc"))
        if os.path.exists(ipc_path):
            return ipc_path
        return None

    elif sys.platform == "win32":
        ipc_path = os.path.join("\\\\", ".", "pipe", "bub.ipc")
        if os.path.exists(ipc_path):
            return ipc_path
        return None
...
def get_dev_ipc_path() -> Optional[str]:
    if os.environ.get("WEB3_PROVIDER_URI", ""):
        ...
        ipc_path = os.path.expanduser(os.path.join(tmpdir, "bub.ipc"))
        if os.path.exists(ipc_path):
            return ipc_path
        return None

    elif sys.platform.startswith("linux") or sys.platform.startswith("freebsd"):
        ipc_path = os.path.expanduser(os.path.join("/tmp", "bub.ipc"))
        if os.path.exists(ipc_path):
            return ipc_path
        return None
...
class IPCProvider(JSONBaseProvider):
    logger = logging.getLogger("bubble.providers.IPCProvider")
    _socket = None
