...
from bubble.tools.benchmark.node import (
    BubBenchmarkFixture,
)
from bubble.tools.benchmark.reporting import (
    print_entry,
    print_footer,
    print_header,
)
...
async def async_unlocked_account(async_w3: AsyncWeb3) -> ChecksumAddress:
    ...
    await async_w3.node.personal.unlock_account(coinbase, KEYFILE_PW)
    return coinbase


def main(logger: logging.Logger, num_calls: int) -> None:
    fixture = BubBenchmarkFixture()
            ...
            methods = [
                {
                    ...
                    "async_exec": lambda: async_w3_http.bub.gas_price,
                },
                {
                    "name": "bub_sendTransaction",
                    "params": {},
                    ...
                    "async_exec": lambda: async_w3_http.bub.send_transaction(
                        {
                            "to": "0xd3CdA913deB6f67967B99D67aCDFa1712C293601",
                ...
                {
                    "name": "bub_blockNumber",
                    "params": {},
                    "exec": lambda: w3_http.eth.block_number,
                    "async_exec": lambda: async_w3_http.bub.block_number,
                },
                {
                    "name": "bub_getBlock",
                    "params": {},
                    "exec": lambda: w3_http.eth.get_block(1),
                    "async_exec": lambda: async_w3_http.bub.get_block(1),
                },
            ]
